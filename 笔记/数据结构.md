...
迪杰斯特拉Dijstra算法：求图中一个顶点到其他**每个顶点**的最短路径。  
...
若i与s直接相连，则dist[i]=g[s] [i]，否则dist[i]=INF；
...
Floyd算法降维优化：
在更新dp[k] [i] [j]时，只需要用到dp[k-1] [] []的数据，1 ~ k-2 的数据不需要，则可以采用滚动二维数组进行降维。
dp[i] [j]：表示顶点 i 到 j 的最短路径长度。  
